...
$html = file_get_contents("http://www.imdb.com/chart/tvmeter?ref_=nv_tvv_mptv_4",false, $context);
...
$newstars = $internalquery->query("//*[contains(concat(' ', normalize-space(@class), ' '), 'credit_summary_item')]");
 if($newstars->length==2){
  ...
  $stars =$newstars->item($j)->getElementsByTagName('span');
	 
		foreach($stars as $star){
			if($star->getAttribute('itemprop')=="actors"){
				$star_array[]=trim($star->nodeValue);
...
//~ die;
    foreach ($result as $shows){
		...
		$filename = time()."-".str_replace(" ","-",$title);
			...
			$filepath = "popular_show_images/".$filename;
	 	  $sel = 'select title from  `imdb_popular_shows` where title="'.$title.'"';
		...
		if($rows<1){
			 copy($image,$filepath);
 	 $insert = 	'INSERT INTO imdb_popular_shows (`title`, `stars`, `description`, `image`, `imdb_rating`) VALUES ("'.$title.'","'.$star.'","'.$description.'","'.$filename.'","'.$imdbRating.'")';
